...
1. 接单方在任务页点"接单"：校验自己已绑收款、不在黑名单、没有被锁定（待确认到账超过 24 小时未处理，§3.10）、并发接单数未超上限、没有在这个任务上进行中的记录（过期/作废满 24 小时后可再接）。通过则锁定一个名额，生成接单记录 `S-XXXXX`，状态 `claimed`，倒计时开始。
...
5. 接单方有 `awaiting_confirm` 记录**超过 24 小时**未处理，就不能发布也不能接新任务（24 小时缓冲是为了让"随手乱标记"不能立刻把接单方全站锁死）。发起 B 类申诉即解锁。24 小时和 72 小时各推一次提醒。**不做自动确认**——自动确认会奖励"标记已付实际没付"的发布方，锁定接单方本身就是最有效的催办。
...
| 锁定（接单方） | 任一记录 `awaiting_confirm` 超过 24 小时未处理 | 不能发布、不能接单 | 确认收到或发起申诉后自动解除 |
...
| 陪审员资格 | 注册 ≥ 14 天；已完成 ≥ 3 条记录（任一角色）；不在黑名单、无冻结/锁定；与双方无关联（90 天内没有与任一方的接单或发布关系、不同 IP 段、不同 `payer_id`）；同时在审 ≤ 3 件；近 20 票与终裁一致率 ≥ 50%（前 10 票不计） |
...
| `awaiting_confirm` | 待确认到账 | 接单方 | 无超时，24h/72h 提醒；期间接单方被锁定 | |
...
| 举报后宽限 | 24h | 固定 | 系统 |
| 确认到账提醒 | 24h、72h | 固定 | 系统 |
| 锁定生效延迟 | 标记后 24h | 固定 | 系统 |
...
额度限制的是"一个从未证明过自己的人最多能让别人白干多少"。**升级只数网关核销的付款**（`confirm_method=gateway`，带 `payer_id`）：手动确认是两个人点两下按钮，自己开两个号就能刷，只展示、不提额。接单方等级与陪审员资格里的"已完成"同样只数网关核销的记录。

| 发布方等级 | 条件 | 敞口上限（未付名额 × 单价之和） | 同时在线任务 |
|---|---|---|---|
| 新手 | 已付款 < 3 单 | 20 U | 1 |
| 常规 | 网关核销 ≥ 3 单、逾期 ≤ 1 | 200 U | 3 |
| 资深 | 网关核销 ≥ 20 单、逾期 ≤ 1、平均付款 < 24h（手动记录以标记时间计） | 2000 U | 10 |
...
| 假标记已付 | 手动模式乱点"已付款"，把接单方锁住 / 洗掉自己的逾期 | 标记不解除冻结、不计信用；接单方 24 小时后才锁定且申诉即解锁；B 类查实即上榜 |
| 收到装没收到 | 接单方讹钱 | 发布方有订单号 + 截图；查实即上榜 |
| 自导自演刷信用 | 自己发任务自己接、两个号互刷 | 同一账号不能接自己的任务；信用只数网关核销（每笔都要真金白银经过网关且留下 `payer_id`）；同 IP 段/同 `payer_id` 的发布-接单对打标不计入 |
| 幽灵接单方占额度 | 收了钱不点确认，永久占发布方敞口 | 待确认满 7 天不再计入敞口（记录仍待确认，B 申诉窗口保留） |
| 换号回归 | 被拉黑后新 X 号注册 | X ID 与 `payer_id` 阻断、账号年龄要求、新手额度；信用只认网关核销所以新号必须从头攒 |
...
派生而非存储：任务剩余名额、发布方是否冻结、接单方是否锁定、信用等级、榜单统计。全部用索引查询实时算，避免状态不一致。
...
- 免审：管理员，以及网关核销付款达到阈值且无逾期、无违约的发布方。
