...
| 陪审员资格 | 注册 ≥ 14 天；已完成 ≥ 3 条记录（任一角色）；不在黑名单、无冻结；与双方无关联（90 天内没有与任一方的接单或发布关系、不同 IP 段与不同浏览器、不同 `payer_id`）；同时在审 ≤ 3 件；近 20 票与终裁一致率 ≥ 50%（前 10 票不计） |
...
额度限制的是"一个从未证明过自己的人最多能让别人白干多少"。**升级数"计信用的完成"**：`status=paid` 且 `confirm_method IN (gateway, manual, admin)`（网关核销、接单方主动确认、裁决完成）且 `self_deal=0`（非自导自演：同账号 / 同付款账户 / 30 天内同一时段（48 小时内重叠）用过同一个 IP，或用过同一个浏览器，接单时和付款结算时各判一次；运营商共用的网段不算，管理员可翻转误判）且任务不是点赞 / 转发。超时自动完成（`auto`）不算——沉默不是确认，也堵住"假标记 + 等 24 小时"刷信用。常规要求来自 ≥2 个不同对手方，资深要求更多不同对手方，发布方资深还要求 ≥3 笔网关核销（有 `payer_id` 锚点才配 2000 U 敞口）。接单方等级与陪审员资格里的"已完成"用同一口径。
...
| 自导自演刷信用 | 自己发任务自己接、两个号互刷 | 同一账号不能接自己的任务；信用不数超时自动完成，且剔除自导自演（同账号 / 同付款账户 / 同一 IP 或同一浏览器）（常规要 2 个不同对手方，资深要 10 个且发布方须有网关核销记录）；同一 IP（同时段）/ 同一浏览器 / 同 `payer_id` 的发布-接单对打标不计入，管理员可翻转 |
...
- 留存期内（记录为「已验证」）发布方可在记录页点「现在就付」：系统立刻复检一次（推文还在、内容一致），通过即进入待付款并通知接单方；按浏览量计价的任务不能提前付款（结算依赖留存期末的浏览量，提前会按接近保底付），付款时限从此刻起算；不达标则作废、无需付款。
