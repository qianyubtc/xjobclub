...
- **信用要靠不同的人**。等级按"计信用的完成"升：网关自动核销、接单方主动确认、裁决完成都算，超时自动完成、点赞 / 转发、自导自演（同账号 / 同付款账户 / 同一 IP 或同一浏览器）不算；常规要来自 ≥2 个不同对手方，资深要 ≥10 个且发布方须有网关核销记录。
...
- **约束**：逾期自动冻结发布方（标记已付不能洗掉）；接单方待确认到账 24 小时未处理视为已收到自动完成（7 天内仍可申诉）；留存不达标累计暂停接单。
...
| 付款时限 | 48 小时；逾期即冻结发布方 |
| 待确认到账 | 标记后 24 小时未处理自动完成（不计信用），自动完成后 7 天内可申诉未收到；等补差中、已举报过的不自动 |
...
- 会话为 HMAC 签名 cookie（`HttpOnly` + `SameSite=Lax`），载荷含密码指纹，改密码即全端下线；登录失败按 IP + 账号限流，不做按用户名的全局锁定。另有一枚随机浏览器标识 cookie（`tdev`，一年）与访问 IP 一起保留 90 天，只用于识别自导自演与陪审员回避，规则页有披露。
